...
f <- as.formula('rate ~ reward * track + (1 | rat/session)')
...
if (save_path == ''){
  ...
  write.csv(out_c, paste0("contrasts_",dep_name,".csv"), row.names = FALSE)
  write.csv(out_emm, paste0("emmeans_",dep_name,".csv"), row.names = FALSE)
  write.csv(as.data.frame(anova(m1_rate)),paste0("anova_",dep_name,".csv"), row.names = TRUE)
} else {
    for (subdir in c("contrasts", "emmeans", "anova")) {
      dir.create(file.path(save_path,subdir),recursive = TRUE,showWarnings = FALSE)
    }
    dir.create(file.path(save_path), showWarnings = FALSE)
    write.csv(out_c, file.path(save_path,paste0("contrasts_",dep_name,".csv")), row.names = FALSE)
    write.csv(out_emm, file.path(save_path,paste0("emmeans_",dep_name,".csv")), row.names = FALSE)
    write.csv(as.data.frame(anova(m1_rate)),file.path(save_path,paste0("anova_",dep_name,".csv")), row.names = TRUE)
}
